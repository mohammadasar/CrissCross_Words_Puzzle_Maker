      ...
      generateCrossword() {
        ...
        for (let i = 0; i < this.gridSize; i++) {
          ...
          for (let j = 0; j < this.gridSize; j++) {
            row.push({ text: "", word: "", active: false, number: 0 }); // Add 'number' property
          }
          this.grid.push(row);
        }
    ...
    const placeWord = (word, row, col, direction) => {
      for (let i = 0; i < word.length; i++) {
        ...
        if (direction === "across") {
          r = row;
          c = col + i;
          console.log("placeword across")
        } else if (direction === "down") {
          r = row + i;
          c = col;
          console.log("worked down placeword");
        
        } 
       
  
        // Check for intersection with existing letter
        if (grid[r][c].text !== word[i] && grid[r][c].text !== "") {
          console.error("Word intersects incorrectly:", word);
          return; // Exit the function if intersection is incorrect
        }
  
        grid[r][c].text = word[i];
        grid[r][c].word = word;
        grid[r][c].active = true; // Mark cell as active
        if (i === 0) grid[r][c].number = word.length; // Assign number to the starting cell of the word
      }
    };
